...
$$
$$\int\frac{1}{(T_{body}-20)}dT=kt+C$$

$$\ln{(T_{body}-20)}=kt+C$$
$$T_{body}=e^{kt+C}+20=Ce^{kt}+20$$

Since $T_{body}=C+20$ at time $t=0$, we can write the equation as
...
$$x(t)=Ce^{-t^2/2}$$
is our general solution to the differential equation.
Now we plug in the initial conditions:
$$1/\sqrt{\pi}=Ce^{-(0^2/2)}=Ce^0=C$$
...
$$ \frac{u^3}{3}=\frac{1}{9}\sin(t)+C$$

$$ u^3=\frac{\sin(t)}{3}+C$$

Now we plug in the initial conditions:
$$ 2^3=\frac{\sin(0)}{3}+C=0+C$$
So $C=8$ and we have
