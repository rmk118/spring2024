...
y2 <- tibble(x=c(0,0,-6,5,0), y=c(1,4,1,0.5,-3), IC=c("V1", "V2", "V3", "V4", "V5"))%>% mutate(pt = paste0("(",x, ",", y, ")"))
```
...
z <- left_join(x,y) %>% filter(x<7 & x>-7 & y<7 & y>-7) %>% left_join(y2 %>% select(IC, pt), by="IC")
...
p1+theme_classic()+
  ...
  geom_point(data=y2, aes(x=x,y=y, color=pt), shape=21, fill="white",size=5, stroke=2)+labs(color="IC")
