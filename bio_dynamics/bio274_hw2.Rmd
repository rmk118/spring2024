...
ggplot()+
  ...
  scale_color_manual(values=pnw_palette(name="Sailboat",n=4,type="discrete"))
```
...
ggplot()+
  geom_line(aes(x=y, y=eval(dy), color="Calculated derivative"), linewidth=3, alpha=0.5)+
  geom_line(aes(x=y, y= 0.3*y*(1-(y/2500))-y/3, color="Original dP/dt"))+
  scale_color_manual(values=c("lightblue", "black"))+
  theme_bw()+
  labs(x="P", y="dP/dt", color=NULL)
```

## Question 2 again - for practice with Bernoulli's method
...
ggplot()+
  ...
  scale_color_manual(values=c("gray",pnw_palette(name="Bay",n=2,type="discrete")))+
  ylim(0, NA)
```
...
The equations needed to use Euler's method are:
$$t_{n+1}=t_n+h$$
$$y_{n+1}=y_n+f(t_n, y_n)h$$
where $f(t_n, y_n)$ is the slope we calculate by plugging $t_n, y_n$ into our original DE.
...
ggplot(data = euler_output)+
  ...
  scale_color_manual(values=pnw_palette(name="Sailboat",n=3,type="discrete"))
```
...
ggplot()+
  geom_line(data=actual_eqn, aes(x=t, y=w, color="Actual solution"), linewidth=1)+
  ...
  theme_light()+
  labs(x="t", y="w", color="h")+
  scale_color_manual(values=c(pnw_palette(name="Sailboat",n=3,type="discrete"), "black"))
```
