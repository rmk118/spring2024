...
We can see that the first term on the right-hand side is the solution to $(vx)_t = k(vx)_{xx}, -\infty<x<\infty$ with the initial condition $v_x(x, 0) = f'(x)$, i.e., it equals $v_x$. Thus, we can subtract $v_x$ from both sides of the above equation and divide by $-2$ to find show that $(\ast)$ is indeed satisfied by
