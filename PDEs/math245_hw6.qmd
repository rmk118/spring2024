...
# (Wednesday morning correction to Problem 5)
...
Here,
\begin{align*}
u(0) &=\text{average of u over } \partial{B}(0,2) \\
&=\frac{1}{2\pi R} \int_{\partial{B}(0,R)} u \; ds \\
&=\frac{1}{4\pi} \int_{\partial{B}(0,R)} u \; ds \\
&=\frac{1}{4\pi} \int_{\theta=0}^{\theta=2\pi} u \cdot R \; d\theta \\
& =\frac{1}{2\pi}\int_0^{2\pi} u\;d\theta \\
& =\frac{1}{2\pi}\int_0^{2\pi} (\sin\theta+1)\;d\theta \\
...
0 &=-\int\limits_{\partial D} g\left( \theta \right)\,d\theta
\end{align*}

Where we used the fact that by (\ref{eqn:prob5}), $\Delta u = 0$ and $\frac{\partial u}{\partial n}=u_r=g(\theta)$. Thus, the necessary condition for the existence of a solution is $$\int\limits_{\partial D} g\left( \theta \right)\,d\theta=0$$
...
$$u_r(r,\theta)=\sum_{n=1}^{\infty}-nr^{-n-1} \left(A_n\cos{(n\theta)}+B_n\sin{(n\theta)}\right) $$

Finally, we use the inhomogeneous BC at $r=a$. Setting $r=a$ in the series above, we require that
$$g(\theta)=\sum_{n=1}^{\infty}-na^{-n-1} \left(A_n\cos{(n\theta)}+B_n\sin{(n\theta)}\right) $$

Since this is a Fourier series for $g(\theta)$, we can now solve for the unknown coefficients:

$$-nA_na^{-n-1}=\frac{1}{\pi} \int_{-\pi}^{\pi}g(\theta)\cos(n\theta)\,d\theta $$
$$-nB_na^{-n-1}=\frac{1}{\pi} \int_{-\pi}^{\pi}g(\theta)\sin(n\theta)\,d\theta $$
...
$$u(r,\theta)=A_0+\sum_{n=1}^{\infty}r^{-n} \left(A_n\cos{(n\theta)}+B_n\sin{(n\theta)}\right) $$
where
$$A_n=-\frac{a^{n+1}}{n\pi} \int_{-\pi}^{\pi}g(\theta) \cos{(n\theta)}\,d\theta $$
$$B_n=-\frac{a^{n+1}}{n\pi} \int_{-\pi}^{\pi}g(\theta) \sin{(n\theta)}\,d\theta $$
\end{tcolorbox}
...
Now replace $u$ and $v$ in this formula with $w$:
...
But from (\ref{eqn:unique2}), $\Delta w=0$ in $D$ and $\frac{\partial w}{\partial n}=0$ on $\partial{D}$, so
