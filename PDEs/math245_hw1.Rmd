...
Set $\eta=xe^t$ and $\xi=x$. Then by the chain rule,
$$u_x= \pdv{u}{x}=\pdv{u}{\xi}\pdv{\xi}{x}+\pdv{u}{\eta}\pdv{\eta}{x}=$$
$$\pdv{u}{\xi}+\pdv{u}{\eta}e^t $$

and similarly
$$u_t= \pdv{u}{t}=\pdv{u}{\xi}\pdv{\xi}{t}+\pdv{u}{\eta}\pdv{\eta}{t}=\pdv{u}{\eta}xe^t $$

Which means that 
$$u_t-xu_x=\pdv{u}{\eta}xe^t -x\left(\pdv{u}{\xi}+\pdv{u}{\eta}e^t \right)=0$$
Assuming $x\neq0$, this means
$$u_\xi=0$$
$$u=\int u_\xi \, d\xi \> + f(\eta)=0+ f(\eta)$$

So we have $u(t,x)=f(\eta)=f(xe^t)$ and we know that $u(0,x)=\frac{1}{1+x^2}$, which means $f(xe^0)=f(x)=\frac{1}{1+x^2}$ and therefore
\begin{tcolorbox}[colback=white, title=Solution]
...
Now we define our new coordinate system as $\eta=y-\frac{x^2}{2}, \, \xi=x$  

Then by the chain rule,
$$u_x= \pdv{u}{x}=\pdv{u}{\xi}\pdv{\xi}{x}+\pdv{u}{\eta}\pdv{\eta}{x}=$$
$$\pdv{u}{\xi}-x\pdv{u}{\eta}$$

and similarly
$$u_y= \pdv{u}{y}=\pdv{u}{\xi}\pdv{\xi}{y}+\pdv{u}{\eta}\pdv{\eta}{y}=$$
$$\pdv{u}{\eta}$$

$$u_x+xu_y=\pdv{u}{\xi}-x\pdv{u}{\eta}+x\pdv{u}{\eta}=\pdv{u}{\xi}$$

And since we know $u_x+xu_y=\left(y-\frac{x^2}{2}\right)^2$, this means we have
$$\pdv{u}{\xi}=\left(y-\frac{x^2}{2}\right)^2=\eta^2$$
Now we integrate with respect to $\xi$ to find
$$u=\xi\eta^2+F(\eta)$$
...
#### (d)
$2x u_x+ (x+1)u_y=y$, for $x>0, u=2y$ on $x=1$

$$\frac{dx}{a}=\frac{dy}{b}=\frac{du}{c} \quad \longrightarrow \quad 
\frac{dx}{2x}=\frac{dy}{x+1}=\frac{du}{y}$$

$$\frac{dx}{2x}=\frac{dy}{x+1}$$
$$\frac{x+1}{2x}\, dx=dy$$
$$\int \frac{1}{2}+\frac{1}{2x}\, dx=\int dy$$

Solving this ODE yields the characteristic curves:
$$y=\frac{x}{2}+\frac{\ln x}{2}-C$$
$$C=\frac{x}{2}+\frac{\ln x}{2}-y$$
...
Now define our new coordinate system as $xi=\frac{x}{2}+\frac{\ln x}{2}-y, \quad \xi=x$.  
Then by the chain rule,
$$u_x= \pdv{u}{x}=\pdv{u}{\xi}\pdv{\xi}{x}+\pdv{u}{\eta}\pdv{\eta}{x}=$$
$$\pdv{u}{\eta}\left(\frac{1}{2}+ \frac{1}{2x}\right) +\pdv{u}{\xi}$$
$$u_y= \pdv{u}{y}=\pdv{u}{\xi}\pdv{\xi}{y}+\pdv{u}{\eta}\pdv{\eta}{y}= -\pdv{u}{\eta}$$
Plugging these into our original PDE,
$$2x\left( \pdv{u}{\eta}\left(\frac{1}{2}+ \frac{1}{2x}\right) +\pdv{u}{\xi} \right)+\left(x+1\right)\left(-\pdv{u}{\eta}\right)=$$
$$\pdv{u}{\eta}\left(x+1\right)+2x \pdv{u}{\xi}-\pdv{u}{\eta}\left(x+1\right)=2x \pdv{u}{\xi}$$
So we must have $2x\pdv{u}{\xi}=y$. Now replace $\xi$ and $\eta$ by $x$ and $y$:
$$2\xi\pdv{u}{\xi}=\frac{\xi}{2}+\frac{\ln\xi}{2}-\eta$$

$$\pdv{u}{\xi}=\frac{1}{4}+\frac{\ln\xi}{4\xi}-\frac{\eta}{2n}$$

Now integrate with respect to $\xi$:
$$u=\frac{\xi}{4}+\frac{(\ln\xi)^2}{8}-\frac{\eta \ln\xi}{2}+F(\eta)$$
$$u(x,y)=\frac{x}{4}+\frac{(\ln x)^2}{8}-\frac{\ln x}{2}\left(\frac{x}{2}+\frac{\ln x}{2}-y \right)+F\left(\frac{x}{2}+\frac{\ln x}{2}-y\right)$$
Now we use the fact that $u(1,y)=2y$:
$$u(1,y)=\frac{1}{4}+F\left(\frac{1}{2}-y\right)=2y$$
$$F\left(\frac{1}{2}-y\right)=2y-\frac{1}{4}$$
So $F(\alpha)=-2\alpha+\frac{3}{4}$ for arbitrary $\alpha$. Then
$$u(x,y)=\frac{x}{4}+\frac{(\ln x)^2}{8}-\frac{\ln x}{2}\left(\frac{x}{2}+\frac{\ln x}{2}-y \right)-x-\ln x +2y+\frac{3}{4}$$
Which can be simplified to
$$u(x,y)=\frac{-3x}{4}-\frac{(\ln x)^2}{8}-\frac{x\ln x}{4}+\frac{y\ln x}{2}-\ln x+2y+\frac{3}{4}$$
\begin{tcolorbox}[colback=white, title=Solution]
$$u(x,y)=-\ln x\left(\frac{x}{4} +\frac{\ln x}{8}+ \frac{y}{2} +1 \right)-\frac{3x}{4}+2y+\frac{3}{4} $$
\end{tcolorbox}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Check solution]
$$u_y=\frac{\ln x}{2}+2$$
$$u_x=\frac{-1}{x} \left(\frac{x}{4} +\frac{\ln x}{8}- \frac{y}{2} +1 \right)+(-\ln x) \left(\frac{1}{4} +\frac{1}{8x}\right)-\frac{3}{4}$$
Which means
$$2xu_x+(x+1)u_y=\left( \frac{-x}{2}-\frac{\ln x}{4}+y-2-\frac {x \ln x}{2}-\frac{\ln x}{4}-\frac{3x}{2}\right)+ \left(\frac {x \ln x}{2}+\frac { \ln x}{2}+2x+2 \right)=$$
$$\left(-\frac {x \ln x}{2}-\frac{\ln x}{2} -2x+-2+y\right)+ \left(\frac {x \ln x}{2}+\frac { \ln x}{2}+2x+2 \right)=y$$
Which matches our original PDE.
\end{tcolorbox}  

#### (e)
$yu_x+xu_y=xy$, for $x,y \geq 0$ with $u(0,y)=\exp(-y^2)$

$$\frac{dx}{a}=\frac{dy}{b}=\frac{du}{c} \quad \longrightarrow \quad 
\frac{dx}{y}=\frac{dy}{x}=\frac{du}{xy}$$


$$\frac{dx}{y}=\frac{dy}{x}\quad \longrightarrow \quad x\;dx=y\;dy\quad \longrightarrow \quad y^2=x^2+C$$

$$y=\sqrt{x^2+C}, \quad C=y^2-x^2$$
Where we kept the positive root for $y$ because it was given that $x,y \geq 0$.

```{r, warning = FALSE, echo=FALSE}
x=seq(-10,10,length.out = 1000)

curves_4e <- tibble(C=c(1:5), seq=list(x)) %>% 
  rowwise() %>% 
  mutate(y=list(sqrt(seq^2+C))) %>% 
  unnest(c(seq,y))

ggplot(data=curves_4e)+
  geom_line(aes(x=seq, y=y, color=as.factor(C)))+
  labs(x="x", y="y", color="C")+
  theme_light()+
  ggtitle("Characteristic curves for Problem 4e")
```

$$\frac{dx}{y}=\frac{du}{xy} \quad\longrightarrow\quad du=x\;dx$$

$$\frac{dy}{x}=\frac{du}{xy} \quad\longrightarrow\quad du=y\;dy$$

$$2\,du= x\,dx+y\,dy=d\left(\frac{x^2+y^2}{2} \right)$$

$$\int du= \frac{1}{4} \int d(x^2+y^2)$$

$$u=\frac{x^2+y^2}{4}+f(C)\quad\longrightarrow\quad u=\frac{x^2+y^2}{4}+f(y^2-x^2)$$

$$u(0,y)=\frac{y^2}{4}+f(y^2)=e^{-y^2}$$


$$f(y^2)=e^{-y^2}-\frac{y^2}{4}$$

$$f(\alpha)=e^{-\alpha}-\frac{\alpha}{4}$$

$$u(x,y)=\frac{x^2+y^2}{4}+ e^{x^2-y^2}+\frac{x^2-y^2}{4}$$

\begin{tcolorbox}[colback=white, title=Solution]
$$u(x,y)=\frac{x^2}{2}+ e^{x^2-y^2}$$
\end{tcolorbox}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Check solution]
$$u_x=x+2xe^{x^2-y^2}, \; u_y=-2ye^{x^2-y^2}$$
$$yu_x+xu_y=xy+2xye^{x^2-y^2}-2xye^{x^2-y^2}=xy$$
Which matches our original PDE.
\end{tcolorbox} 