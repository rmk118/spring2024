...
what is $\lim\limits_{x \to \infty}u(t,x)$?
...
Thus $u(t, \tan{(t+C)})=u(0, \tan{(0+C)})=u(0, C)$ is independent of $t$. Putting $x=\tan{(t+C)}$ and $C=\arctan{(x)}-t$, we have
$$u(t, x)=u(0,\arctan{(x)}-t)$$
It follows that $u(t,x)=f(\arctan{(x)}-t)$. And since we are given that $u(0,x)=\arctan{(x)}$, we have $f(\arctan{(x)}-0)=\arctan{(x)}$ so that $f(w)=w$ for any $w$, yielding our solution of 
$$u(t,x)=\arctan{(x)}-t$$
