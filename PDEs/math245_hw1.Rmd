...
### Question 2
(a) 2nd-order linear homogenous  
(b) 4th-order linear inhomogenous  
(c) 2nd-order quasi-linear homogeneous  
(d) We can rewrite this as $u^{-5}u_{xx}+u^{-5}u_{yy}+u^{-4}f(x,y)=g(x,y)$ to see that it is a 2nd-order quasi-linear inhomogeneous PDE.
...
We can check our solution of $u(t,x)=\arctan{(x)}-t$ by differentiating, since $u_t=-1, u_x=\frac{1}{1+x^2}$, that means $u_t+\left(1+x^2\right)u_x=-1+1=0$.
...
```{r, warning = FALSE}
curves_4a <- tibble(C=c(1:5), seq=list(x)) %>% 
  rowwise() %>% 
  mutate(y=list(C*exp(-seq))) %>% 
  unnest(c(seq,y))

ggplot(data=curves_4a)+
...
So we have $u(t,x)=f(\xi)=f(xe^t)$ and we know that $u(0,x)=\frac{1}{1+x^2}$, which means $f(xe^0)=f(x)=\frac{1}{1+x^2}$ and therefore $u(t,x)=\frac{1}{1+\left(xe^t\right)^2}$.
...
We can see that $u(t,x)$ will equal 1 at $x=0$ regardless of the value of $t$, but at all other values of $x$, $\lim\limits_{t \to \infty}u(t,x)=0$.