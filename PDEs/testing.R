...
mom2 <- mom2 %>% mutate(Date=mdy(Date), Time=ms(Time))

ggplot(data=mom2, aes(x=Date, y=Time))+
  geom_point()+
  theme_classic()+
  geom_smooth(method="lm")+
  stat_cor(aes(label = after_stat(r.label)),  r.accuracy = 0.001, label.x.npc=0.1, label.y.npc = "bottom")+
  scale_y_time(breaks = c(ms("27:30","27:00", "26:30", "26:00", "25:30")), labels = c("27:30","27:00", "26:30", "26:00" ,"25:30"))+theme(axis.title.y = element_text(margin = margin(r = 10)))
